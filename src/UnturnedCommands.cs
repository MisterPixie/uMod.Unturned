    ...
    public partial class UnturnedCore
    {
        ...
        private void VersionCommand(IPlayer player, string command, string[] args)
        {
            if (player.IsServer)
            {
                ...
                $"Branch: {BuildInfo.Current.Scm.Branch}\nOxide.Unturned Version: {UnturnedExtension.AssemblyVersion}");*/ // TODO: Implement server version
            }
            else
            {
                string format = Covalence.FormatText(lang.GetMessage("Version", this, player.Id));
                player.Reply(string.Format(format, UnturnedExtension.AssemblyVersion, Covalence.GameName, Server.Version, Server.Protocol));
            }
        }
